...
describe('2. Testa se os elementos da página de detalhes estão visíveis e seus atributos', () => {
  afterEach(() => cleanup());
...
describe('3. Testes do botão de iniciar a receita', () => {
  ...
  test('3.1. Testa se o botão não é renderizado caso a receita esteja feita', async () => {
    ...
    const startBttDisplayed = buttons.some((btt) => btt.textContent === 'Start Recipe' || btt.textContent === 'Continue Recipe');
    expect(startBttDisplayed).toBe(false);
  });

  test('3.2. Se uma receita já foi iniciada, o botão apresenta o texto "Continue Recipe"', async () => {
    await changeLocalStorage('inProgressRecipes', JSON.stringify({ meals: { ['52772']: [] } }), 'replace');
    global.fetch = jest.fn().mockResolvedValueOnce({
      json: jest.fn().mockResolvedValue(foodDetailed),
    }).mockResolvedValueOnce({
      json: jest.fn().mockResolvedValue(drinks),
